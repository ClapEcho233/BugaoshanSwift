...
// MARK: - 时长设置（set_duration_page.dart）

/// 每课时长 / 课间时长 → 重建当前课表 timeSlots
struct SetDurationPage: View {
    @EnvironmentObject private var config: AppConfig
    @EnvironmentObject private var courseProvider: CourseProvider

    @State private var courseDuration: Double = 45
    @State private var breakDuration: Double = 10
            ...
            Section {
                VStack(alignment: .leading) {
                    HStack {
                        Text("每课时长")
                        Spacer()
                        Text("\(Int(courseDuration)) 分钟").foregroundStyle(.secondary)
                    }
                    Slider(value: $courseDuration, in: 30...60, step: 5)
                }
                VStack(alignment: .leading) {
                    HStack {
                        Text("课间时长")
                        Spacer()
                        Text("\(Int(breakDuration)) 分钟").foregroundStyle(.secondary)
                    }
                    Slider(value: $breakDuration, in: 5...30, step: 5)
                }
                Button("应用到当前课表") {
                    Task { await apply() }
                }
                ...
                Text("修改后按上午 4 节 / 下午 5 节 / 晚上 3 节重建节次时间表")
            }
        }
        .navigationTitle("时长设置")
        .navigationBarTitleDisplayMode(.inline)
        .onAppear {
            courseDuration = Double(courseProvider.config?.courseDuration ?? 45)
            breakDuration = Double(courseProvider.config?.breakDuration ?? 10)
        }
        .alert("提示", isPresented: Binding(
            get: { savedMessage != nil }, set: { if !$0 { savedMessage = nil } }
        )) {
    ...
    private func apply() async {
        guard var scheduleConfig = courseProvider.config else {
            savedMessage = "当前没有课表"
            return
        }
        scheduleConfig.courseDuration = Int(courseDuration)
        scheduleConfig.breakDuration = Int(breakDuration)
        scheduleConfig.timeSlots = ScheduleConfig.defaultTimeSlots(
            morning: scheduleConfig.morningSections,
            afternoon: scheduleConfig.afternoonSections,
            evening: scheduleConfig.eveningSections,
            courseDuration: Int(courseDuration),
            breakDuration: Int(breakDuration)
        )
        await courseProvider.saveConfig(scheduleConfig)
        savedMessage = "已应用"
    }
}
